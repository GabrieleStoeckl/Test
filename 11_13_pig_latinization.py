...
txt = """You would never guess what can happen when you jump into a seemingly shallow puddle at night time!
...
#    if txt_test[y] not in dividers:
#        result_test += txt_test[:y]
#        txt_test = txt_test.lstrip(txt_test[:y]) 
#        break
#print(result_test, txt_test)


for n in range (1, iter):
    for i in range(0,length):
        if txt[i] in dividers:
            sign = txt[i]
            break
    wordend = txt.find(sign)
    txt1 = txt[:wordend]
    txt2 = txt1.lstrip(txt[0])
    txt2 += txt[0] + "ay"
    result += txt2 
    txt = txt.lstrip(txt[:wordend])
    for y in range (0,length):
...
#wie bekomme ich die line divider noch hin und die Anführungszeichen?
